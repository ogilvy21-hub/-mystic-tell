...
// 🔧 미스틱텔 개선된 핵심 수정사항
// 테스트 결과를 바탕으로 한 보완 코드

// 1️⃣ 이벤트 리스너 중복 방지 시스템
const EventManager = {
  boundElements: new WeakSet(),
  
  bindOnce(element, event, handler, options = false) {
    if (this.boundElements.has(element)) return false;
...
function initializeButtonEvents() {
  console.log('🔧 버튼 이벤트 초기화 시작...');
  
  // 행운번호 버튼 - 개별 처리
  const btnLotto = document.getElementById('btnLotto');
  if (btnLotto && EventManager.bindOnce(btnLotto, 'click', handleLottoClick)) {
    console.log('✅ 행운번호 버튼 이벤트 등록');
  }
  
  // 궁합 버튼 - 개별 처리  
  const btnMatch = document.getElementById('btnMatch');
  if (btnMatch && EventManager.bindOnce(btnMatch, 'click', handleMatchClick)) {
    console.log('✅ 궁합 버튼 이벤트 등록');
  }
  
  // 신년운세 버튼 - 개별 처리
  const btnYear = document.getElementById('btnYear');
  if (btnYear && EventManager.bindOnce(btnYear, 'click', handleYearClick)) {
    console.log('✅ 신년운세 버튼 이벤트 등록');
  }
  
  // 오늘의 운세 버튼 - 개별 처리
  const btnToday = document.getElementById('btnToday');
  if (btnToday && EventManager.bindOnce(btnToday, 'click', handleTodayClick)) {
    console.log('✅ 오늘운세 버튼 이벤트 등록');
  }
  
  // 사주 버튼 - 개별 처리
  const btnSaju = document.getElementById('btnSaju');
  if (btnSaju && EventManager.bindOnce(btnSaju, 'click', handleSajuClick)) {
    console.log('✅ 사주 버튼 이벤트 등록');
  }
  
  // 특별 서비스 버튼들 (신년운세, 손금보기)
  initializeSpecialServiceButtons();
  
  console.log('🎯 모든 버튼 이벤트 초기화 완료');
}

// 3️⃣ 개별 버튼 핸들러 함수들
function handleLottoClick(e) {
  e.preventDefault();
  console.log('🎲 행운번호 버튼 클릭됨');
  
  const birthInput = document.getElementById('lotto-birth');
  const birth = birthInput ? birthInput.value : '';
  
  console.log('생년월일 입력값:', birth);
  
  try {
    const result = generateEnhancedLotto(birth);
    showLottoResult(result, birth);
  } catch (error) {
    console.error('행운번호 생성 오류:', error);
...
function handleMatchClick(e) {
  e.preventDefault();
  console.log('💕 궁합 버튼 클릭됨');
  
  const matchA = document.getElementById('match-a');
  const matchB = document.getElementById('match-b');
  const birth1 = matchA ? matchA.value : '';
  const birth2 = matchB ? matchB.value : '';
  
  if (!birth1 || !birth2) {
    alert('두 분의 생년월일을 모두 입력하세요.');
    return;
  }
  
  try {
    const result = calcEnhancedMatch(birth1, birth2);
    showMatchResult(result, birth1, birth2);
  } catch (error) {
...
function handleYearClick(e) {
  e.preventDefault();
  console.log('🎊 신년운세 버튼 클릭됨');
  
  const yearBirth = document.getElementById('year-birth');
  const birth = yearBirth ? yearBirth.value : '';
  ...
  try {
    const result = calcEnhancedYear(birth);
    showYearResult(result, birth);
  } catch (error) {
...
function handleTodayClick(e) {
  e.preventDefault();
  console.log('🌟 오늘운세 버튼 클릭됨');
  
  const todayBirth = document.getElementById('today-birth');
  const birth = todayBirth ? todayBirth.value : '';
  ...
  try {
    const result = calcEnhancedDailyFortune(birth);
    showTodayResult(result, birth);
  } catch (error) {
    console.error('오늘운세 계산 오류:', error);
    alert('오늘운세 계산 중 오류가 발생했습니다.');
  }
}

function handleSajuClick(e) {
  e.preventDefault();
  console.log('🔮 사주 버튼 클릭됨');
  
  const sajuBirth = document.getElementById('saju-birth');
  const sajuTime = document.getElementById('saju-time');
  const sajuName = document.getElementById('saju-name');
  
  const birth = sajuBirth ? sajuBirth.value : '';
  const time = sajuTime ? sajuTime.value : '';
  const name = sajuName ? sajuName.value : '';
  ...
  try {
    const result = computeBaZi(birth, time, 'solar', false);
    const enhancedResult = buildEnhancedSajuResult(result, name);
    showSajuResult(enhancedResult);
  } catch (error) {
...
function generateEnhancedLotto(birthdate = '') {
  console.log('🎲 행운번호 생성 시작, 생년월일:', birthdate);
  ...
  if (birthdate) {
    ...
    for (let i = 0; i < cleanBirth.length; i++) {
      seed += parseInt(cleanBirth[i]) * (i + 1);
    }
  } else {
    seed = Date.now();
  }
  
  console.log('시드값:', seed);
  
  // 시드 기반 랜덤 번호 생성 (중복 방지)
  for (let i = 0; i < 6; i++) {
    let attempts = 0;
    let num;
    ...
    do {
      ...
      num = Math.floor((seed / 233280) * 45) + 1;
      attempts++;
  ...
  // 보너스 번호 생성 (본 번호와 중복되지 않게)
  let bonus;
  do {
    seed = (seed * 9301 + 49297) % 233280;
    bonus = Math.floor((seed / 233280) * 45) + 1;
  } while (numbers.includes(bonus));
  
  const result = { numbers, bonus };
  console.log('🎯 생성된 행운번호:', result);
  
  return result;
}

// 5️⃣ 결과 표시 함수들
function showLottoResult(result, birth) {
  const resultText = `
🎲 미스틱텔 행운번호

🎯 추천번호: ${result.numbers.map(n => String(n).padStart(2,'0')).join(' - ')}
⭐ 보너스: ${String(result.bonus).padStart(2,'0')}

${birth ? '🔮 맞춤 생성 (생년월일: ' + birth + ')' : '🎲 랜덤 생성'}

🍀 행운을 빕니다!
  `;
  
  alert(resultText);
  console.log('✅ 행운번호 결과 표시 완료');
}

function showMatchResult(result, birth1, birth2) {
  const resultText = `
💕 궁합 결과

👫 생년월일: ${birth1} ↔ ${birth2}
💯 궁합 점수: ${result.score}/100점

📝 해석: ${result.text}

💑 ${result.score >= 70 ? '훌륭한 궁합입니다!' : '서로 노력하면 좋은 관계가 될 수 있어요!'}
  `;
  
  alert(resultText);
  console.log('✅ 궁합 결과 표시 완료');
}

function showYearResult(result, birth) {
  const resultText = `
🎊 2025년 신년 운세
...
🎯 새해 복 많이 받으세요!
  ...
  alert(resultText);
  console.log('✅ 신년운세 결과 표시 완료');
}

function showTodayResult(result, birth) {
  const categories = Object.values(result.categories);
  const avgScore = Math.round(categories.reduce((sum, cat) => sum + cat.score, 0) / categories.length);
...
🍀 오늘의 키워드: ${categories[0].message}

✨ 좋은 하루 보내세요!
  `;
  
  alert(resultText);
  console.log('✅ 오늘운세 결과 표시 완료');
}

function showSajuResult(enhancedResult) {
  // 사주 결과는 기존 openSheet 함수 사용
  if (typeof openSheet === 'function') {
    openSheet('정통 사주 해석', enhancedResult, {type: 'saju'});
  } else {
    alert('사주 해석이 완료되었습니다. 자세한 내용은 결과창을 확인하세요.');
  }
  console.log('✅ 사주 결과 표시 완료');
}

// 6️⃣ 개선된 궁합 계산기
function calcEnhancedMatch(birth1, birth2) {
  if (!birth1 || !birth2) {
    return {score: null, text: '두 생년월일을 모두 입력하세요.'};
  }
  
  const seed = (birth1 + birth2).replaceAll('-', '');
  let hash = 0;
  ...
  let text;
  if (score >= 85) {
    text = '환상의 궁합! 서로를 완벽하게 보완합니다.';
  } else if (score >= 70) {
    text = '매우 좋은 궁합! 서로에게 큰 힘이 됩니다.';
  } else if (score >= 55) {
    text = '좋은 궁합! 대화와 소통이 잘 맞습니다.';
  } else if (score >= 40) {
    text = '보통 궁합. 서로 노력하면 좋은 관계가 됩니다.';
  } else if (score >= 25) {
    text = '조금 어려운 궁합. 인내와 배려가 필요합니다.';
  } else {
    text = '많은 노력이 필요한 궁합. 천천히 이해해가세요.';
  }
...
// 7️⃣ 개선된 신년운세 계산기
function calcEnhancedYear(birth) {
  if (!birth) {
    return {idx: null, text: '생년월일을 입력하세요.'};
  }
  
  const [year, month, day] = birth.split('-').map(Number);
  const seed = (year + month + day + 2025) % 12;
  
  const yearTexts = [
    '🌟 도약의 해: 새로운 기회가 많이 찾아올 것입니다. 과감한 도전이 성공으로 이어집니다.',
    '📈 성장의 해: 실력 향상과 발전이 기대됩니다. 배움에 투자하면 큰 성과를 얻습니다.',
    '⚖️ 안정의 해: 차근차근 기반을 다지는 시기입니다. 꾸준함이 가장 큰 무기입니다.',
    '🔄 변화의 해: 큰 전환점이 있을 수 있습니다. 변화를 두려워하지 마세요.',
    '🏆 수확의 해: 그동안의 노력이 결실을 맺을 것입니다. 성취의 기쁨을 누리세요.',
    '🎯 준비의 해: 미래를 위한 준비를 하는 시기입니다. 계획을 세우고 차근차근 실행하세요.',
    '💫 행운의 해: 예상치 못한 기회와 행운이 찾아올 것입니다.',
    '🤝 관계의 해: 인간관계가 발전하고 좋은 인연을 만날 수 있습니다.',
    '💰 재물의 해: 경제적으로 안정되고 수입이 증가할 가능성이 높습니다.',
    '❤️ 사랑의 해: 연애운이 상승하고 좋은 만남이 기다리고 있습니다.',
    '🎨 창조의 해: 창의적인 활동에서 큰 성과를 거둘 수 있습니다.',
    '🧘 성찰의 해: 내면의 성장과 지혜를 얻는 소중한 시간입니다.'
  ];
  
  const text = yearTexts[seed];
  
  return {idx: seed, text};
}

// 8️⃣ 특별 서비스 버튼 초기화 (신년운세, 손금보기 카드들)
function initializeSpecialServiceButtons() {
  console.log('🎯 특별 서비스 버튼 초기화...');
  
  // 신년운세 카드 버튼들
  const yearCards = document.querySelectorAll('[data-route*="year"], .special-item');
  yearCards.forEach(card => {
    const text = card.textContent?.trim();
    if (text && (text.includes('신년') || text.includes('2025'))) {
      EventManager.bindOnce(card, 'click', (e) => {
        e.preventDefault();
        showSpecialYearFortune();
      });
      console.log('✅ 신년운세 카드 이벤트 등록');
    }
  });
  
  // 손금보기 카드 버튼들
  const palmCards = document.querySelectorAll('[data-route*="palm"], .special-item');
  palmCards.forEach(card => {
    const text = card.textContent?.trim();
    if (text && text.includes('손금')) {
      EventManager.bindOnce(card, 'click', (e) => {
        e.preventDefault();
        showPalmComingSoon();
      });
      console.log('✅ 손금보기 카드 이벤트 등록');
    }
  });
}

function showSpecialYearFortune() {
  const resultText = `
🎊 2025년 특별 신년운세

✨ 올해는 새로운 시작과 희망의 해입니다!
...
🍀 행운의 조언:
긍정적인 마음가짐으로 새로운 기회를 맞이하세요.
꾸준한 노력이 큰 성과로 이어질 것입니다.
...
function enhancePageNavigation() {
  console.log('🔧 페이지 네비게이션 강화...');
  
  // 상단 네비게이션 버튼들
  const navButtons = document.querySelectorAll('a[href*="#"], nav a, .nav-item');
  navButtons.forEach(btn => {
    EventManager.bindOnce(btn, 'click', handleNavigation);
  });
  
  // 하단 네비게이션 버튼들  
  const bottomNavButtons = document.querySelectorAll('.bottom-nav a, #bottomNav a');
  bottomNavButtons.forEach(btn => {
    EventManager.bindOnce(btn, 'click', handleNavigation);
  });
  
  console.log('✅ 페이지 네비게이션 강화 완료');
}
...
function handleNavigation(e) {
  ...
  console.log(`🧭 네비게이션 클릭: "${text}" href="${href}"`);
  
  // 기존 페이지 전환 로직 실행
  if (typeof setActiveTab === 'function' && typeof showFortuneView === 'function') {
    if (href === '#/home' || text === '홈') {
      setActiveTab('home');
    } else if (href === '#/fortune/today' || text === '오늘의 운세') {
      setActiveTab('fortune');
      showFortuneView('fortune-today');
    } else if (href === '#/fortune/saju' || text === '정통 사주') {
      setActiveTab('fortune');
      showFortuneView('fortune-saju');
    }
  }
}

// 🔟 전체 초기화 함수
function initializeEnhancedMysticTell() {
  console.log('🚀 미스틱텔 개선된 초기화 시작...');
  ...
  try {
    ...
    window.addEventListener('error', (e) => {
      console.error('🚨 미스틱텔 오류:', e.error);
    });
    
    console.log('✅ 미스틱텔 개선된 초기화 완료!');
    console.log('🎯 모든 버튼이 개별 기능으로 작동합니다.');
...
// 📋 실행부: DOM 로드 후 초기화
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initializeEnhancedMysticTell);
} else {
  initializeEnhancedMysticTell();
}
...
const observer = new MutationObserver((mutations) => {
  ...
  mutations.forEach((mutation) => {
    if (mutation.type === 'childList') {
      const hasNewButtons = Array.from(mutation.addedNodes).some(node => 
        node.nodeType === 1 && 
        (node.matches?.('button, .btn, [data-route]') || 
         node.querySelector?.('button, .btn, [data-route]'))
      );
  ...
  if (shouldReinit) {
    console.log('🔄 새 요소 감지 - 이벤트 재초기화');
    setTimeout(initializeButtonEvents, 300);
  }
});
...
console.log('🎉 미스틱텔 개선 코드 로드 완료!');
console.log('📋 적용된 개선사항:');
console.log('   ✅ 이벤트 리스너 중복 방지');
console.log('   ✅ 개별 버튼 기능 명확히 분리');
console.log('   ✅ 행운번호 생성 알고리즘 개선');
console.log('   ✅ 에러 처리 강화');
console.log('   ✅ 페이지 네비게이션 개선');
